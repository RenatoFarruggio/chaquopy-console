from ECT_FCTRL_DB.logStore.transconn.database_connector import DatabaseConnector
import cbor
import hashlib
...
:type  received_extensions: bytes (cbor)
"""
    ...
    for i, val in enumerate(i_want_extensions_list):
        ...
        if verify_validation(val, appended_events_list[0]):
            for ev in appended_events_list:
                app_name = cbor.loads(ev)
                app_name = cbor.loads(app_name[2])
